...
// src/actions/scanWallet.ts
import {
  elizaLogger as elizaLogger2
} from "@ai16z/eliza";
import { isAxiosError } from "axios";

// src/services/topwallets-api.ts
...
var TopWalletsAPI = class _TopWalletsAPI {
  client;
  static instance;
  constructor() {
  ...
  static getInstance() {
    if (!_TopWalletsAPI.instance) {
      _TopWalletsAPI.instance = new _TopWalletsAPI();
    }
    return _TopWalletsAPI.instance;
  }
...
var scanWalletAction = {
  ...
  description: "Scan a Solana wallet address to get detailed pnl statistics and profile information",
  ...
  handler: async (runtime, message, state, _options = {}, callback) => {
    ...
    try {
      ...
      const profileItems = [
        ...
        walletData.twitter_url && `\u2022 Twitter: ${walletData.twitter_url}`,
        walletData.type === "kols" && "\u2022 Known Trader \u{1F31F}"
      ].filter(Boolean);
      const profileText = profileItems.length > 0 ? ["\u{1F464} Profile:", ...profileItems].join("\n") : "";
      const analysisText = `\u{1F4B0} Performance Analysis (Last 30 Days):
...
\u2022 Total Invested: ${walletData.totalInvestedFormatted || "Unknown"}`;
      const recentTokensText = walletData.recentTokens.length > 0 ? `

\u{1F504} Recent Token Activity:${walletData.recentTokens.slice(0, 3).map(
        (token) => `
...
\u{1F50D} View complete analysis: https://www.topwallets.ai/solana/wallet/${address}`;
      elizaLogger2.log("Wallet scan successful", {
        address,
        profileText,
        analysisText
      });
      ...
      elizaLogger2.error("Wallet scan error", { error });
      const errorMessage = isAxiosError(error) ? `Failed to scan wallet: ${error.response?.data?.message || error.message}` : "An unexpected error occurred while scanning the wallet.";
      await callback({
        text: errorMessage,
        action: "WALLET_SCAN_RESPONSE"
      });
...
// src/providers/trendingTokens.ts
import {
  composeContext,
...
// src/types.ts
var TrendingTokenSchema = z.object({
...
var isTrendingTokenParams = (object) => {
...
// src/providers/trendingTokens.ts
var shouldShowTrendingTemplate = `# Task: Determine if the user is requesting trending or popular tokens information.
...
var extractParamsTemplate = `# Task: Extract trending tokens request parameters from the conversation.
...
\`\`\``;
var trendingTokensProvider = {
  get: async (runtime, message, state) => {
    try {
      elizaLogger3.debug("Starting trending tokens provider");
      elizaLogger3.debug("Input message:", message.content.text);
      const text = message.content.text;
      const solanaAddressRegex = /[1-9A-HJ-NP-Za-km-z]{32,44}/g;
      ...
      elizaLogger3.debug("Last message:", lastMessage);
      ...
      elizaLogger3.debug("Should show trending?", shouldShowTrending);
      ...
      elizaLogger3.debug("Generated params:", params.object);
      if (!isTrendingTokenParams(params.object)) {
        elizaLogger3.error("Invalid params generated:", params.object);
      ...
      elizaLogger3.debug("Checking cache with key:", cacheKey);
      let trendingTokensResponse = await runtime.cacheManager.get(cacheKey);
      if (!trendingTokensResponse) {
        elizaLogger3.debug("Cache miss, fetching from API");
        const api = TopWalletsAPI.getInstance();
        trendingTokensResponse = await api.getTrendingTokens({
          timeframe: params.object.timeframe,
          count: Math.min(Math.max(1, params.object.count), 20)
          // Ensure count is between 1 and 20
        });
        elizaLogger3.debug("API response:", trendingTokensResponse);
        ...
        elizaLogger3.debug("Cache hit");
      }
      const formattedTokens = trendingTokensResponse.data.tokens.map((token, index) => {
        const price = token.price ? `$${token.price.toFixed(4)}` : "N/A";
        const marketCap = token.marketCap ? `$${(token.marketCap / 1e6).toFixed(2)}M` : "N/A";
        const liquidity = token.liquidity ? `$${(token.liquidity / 1e3).toFixed(2)}K` : "N/A";
...
Last updated: ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}
`.trim();
    } catch (error) {
      elizaLogger3.error("Trending token provider error:", {
        error,
...
function formatPercentages(percentages, nb = 3) {
...
function analyzeMetrics(token) {
  const metrics = [];
  if (token.isRugged) {
    metrics.push(
      "\u{1F6A8} WARNING: This token has been identified as potentially rugged!"
    );
  }
  const timeframes = [
    "1m",
  ...
  timeframes.forEach((timeframe) => {
    ...
    if (change && Math.abs(change) > 5) {
      metrics.push(
        `${change > 0 ? "\u{1F4C8}" : "\u{1F4C9}"} ${Math.abs(change).toFixed(2)}% ${change > 0 ? "gain" : "loss"} in ${timeframe}`
      );
    }
  });
  if (token.liquidity) {
    if (token.liquidity < 1e4) {
      metrics.push("\u{1F6A8} Very low liquidity - high risk of price impact");
    } else if (token.liquidity < 5e4) {
      metrics.push("\u26A0\uFE0F Low liquidity - moderate risk of price impact");
    } else if (token.liquidity < 1e5) {
      metrics.push("\u2139\uFE0F Moderate liquidity");
    }
  }
  if (token.riskScore >= 7) {
    metrics.push("\u{1F6A8} High risk score - exercise extreme caution");
...
function structureTokenData(token) {
  const getLiquidityStatus = (liquidity) => {
    if (!liquidity || liquidity < 1e4) return "CRITICAL";
    if (liquidity < 5e4) return "LOW";
    if (liquidity < 1e5) return "DECENT";
    return "SOLID";
  };
  const getMarketCapCategory = (marketCap) => {
    if (!marketCap || marketCap < 1e5) return "MICRO_CAP";
    if (marketCap < 1e6) return "NANO_CAP";
    if (marketCap < 5e6) return "SMALL_CAP";
    return "BASED";
  };
  const significantMoves = Object.entries(token.priceChange).filter(([_, change]) => change && Math.abs(change) > 5).map(([timeframe, change]) => ({
    timeframe,
    change: Math.abs(change),
    direction: change > 0 ? "gain" : "loss"
  }));
  const kols = token.topWallets?.filter((w) => w.type === "kols") || [];
  const kolHandles = kols.map((k) => extractTwitterHandle(k.twitter_url) || k.name || k.address).filter(Boolean);
  return {
    basicInfo: {
      name: token.name,
      symbol: token.symbol,
...
{{/if}}
`;
async function generateAIAnalysis(token, state, runtime) {
  const tokenData = structureTokenData(token);
  const hasValidDescription = token.description && token.description.length >= 30;
  const analysisState = {
    ...state,
    tokenName: token.name,
    tokenDescription: hasValidDescription ? token.description : "No detailed description available",
    ...
    priceChanges: tokenData.priceAction.significantMoves.map(
      (m) => `- ${m.timeframe}: ${m.change.toFixed(2)}% ${m.direction}`
    ).join("\n"),
...
// src/services/birdeye-api.ts
import { elizaLogger as elizaLogger4 } from "@ai16z/eliza";
import axios2 from "axios";
var BirdeyeApi = class _BirdeyeApi {
  client;
  static instance;
  constructor() {
  ...
  static getInstance() {
    if (!_BirdeyeApi.instance) {
      _BirdeyeApi.instance = new _BirdeyeApi();
    }
    return _BirdeyeApi.instance;
  }
  async getOHLCV(tokenAddress, type = "1D") {
    try {
      const response = await this.client.get(
        `/defi/ohlcv?address=${tokenAddress}&type=${type}&time_from=0&time_to=10000000000`
      );
      if (!response.data.success || response.data.data.items.length === 0) {
        elizaLogger4.warn("Token info fetch failed", {
          error: response.data.message
        });
        throw new Error(response.data.message);
      }
      ...
      elizaLogger4.error("Token info error", { error });
      throw error;
    }
  }
  ...
  async getTopHolders(tokenAddress, nb = 10, offset = 0) {
    try {
      const response = await this.client.get(
      ...
      elizaLogger4.log("holders", response.data);
      return response.data;
    } catch (error) {
      elizaLogger4.error("Token info error", { error });
      throw error;
...
// src/services/dexscreener-api.ts
import { elizaLogger as elizaLogger5 } from "@ai16z/eliza";
import axios3 from "axios";
var DexScreenerApi = class _DexScreenerApi {
  client;
  static instance;
  constructor() {
  ...
  static getInstance() {
    if (!_DexScreenerApi.instance) {
      _DexScreenerApi.instance = new _DexScreenerApi();
    }
    return _DexScreenerApi.instance;
  }
  async getTokenInfo(tokenAddress) {
    try {
      ...
      if (!response.data.pairs || response.data.pairs.length === 0) {
        elizaLogger5.warn("Token info fetch failed", {
      ...
      elizaLogger5.error("Token info error", { error });
      throw error;
    }
  }
};
...
function getRiskScoreEmoji(riskScore) {
  if (riskScore >= 8) {
    return "\u{1F7E9}";
  } else if (riskScore >= 4) {
    return "\u{1F7E7}";
...
function getAth(data) {
  ...
  return {
    high: ath.high,
    date: ath.unixTime * 1e3
  };
}
function formatWalletName(wallet) {
  const name = wallet.name || wallet.address.slice(0, 4) + "..." + wallet.address.slice(-4);
  return wallet.type === "kols" ? `\u2B50 ${name}` : name;
}
async function getTopPercentHolders(tokenAddress) {
  try {
    ...
    console.error("Token scan error", {
      ...
      errorMessage: isAxiosError2(error) ? error.response?.data?.message || error.message : error instanceof Error ? error.message : "Unknown error",
      isAxiosError: isAxiosError2(error)
    });
    return [];
  }
}
var scanTokenAction = {
  ...
  description: "Analyze a Solana token to get detailed price, liquidity, and risk metrics",
  validate: async (runtime, message) => {
    const text = message.content.text;
    const solanaAddressRegex = /[1-9A-HJ-NP-Za-km-z]{32,44}/g;
    if (!text || typeof text !== "string") {
    ...
    return solanaAddressRegex.test(text) || /\$[A-Za-z]+/i.test(text) || /token|price|analysis/i.test(text);
  },
  handler: async (runtime, message, state, _options = {}, callback) => {
    if (!callback) {
    ...
    const solanaAddressRegex = /[1-9A-HJ-NP-Za-km-z]{32,44}/g;
    const matches = text.match(solanaAddressRegex);
    elizaLogger6.debug("Processing scanToken request", {
      text,
      matches,
    ...
    try {
      ...
      let responseOhclv;
      if (new Date(responseDex.pairCreatedAt).getTime() > Date.now() - 24 * 60 * 60 * 1e3) {
      ...
      elizaLogger6.debug("Token data received", {
        address,
...
`;
      analysisText += `\u{1F4CA} Token Analysis:
`;
...
`;
      ...
      analysisText += `\u{1F4B0} Price: $${tokenData.price?.toFixed(6) || "N/A"}  \u21E8 ATH: $${priceAth.high.toFixed(6)} [${getTimeAgo(
        priceAth.date
      )}]
`;
      analysisText += `\u{1FA99} Market Cap: $${formatNumber(
        tokenData.marketCap
      )}
`;
      analysisText += `\u{1F48E} FDV: $${formatNumber(responseDex.fdv)}
`;
      analysisText += `${getRiskScoreEmoji(
        tokenData.riskScore
      )} Risk Score: ${tokenData.riskScore}/10
`;
      analysisText += `\u{1F4A6} Liq: $${formatNumber(tokenData.liquidity)}
`;
      analysisText += `\u{1F4CA} Vol: $${formatNumber(
        responseDex.volume.h24
      )} \u{1F570}\uFE0F Age: ${getTimeAgo(responseDex.pairCreatedAt)}
`;
      if (source !== "telegram") {
        const priceChange24h = tokenData.priceChange["24h"];
        const changeIcon = priceChange24h >= 0 ? "\u{1F4C8}" : "\u{1F4C9}";
        analysisText += `\u{1F4C8} 24h Change: ${changeIcon} ${priceChange24h?.toFixed(2) || "0"}%
`;
      }
      if (tokenData.isRugged) {
        analysisText += `\u2022 \u{1F6A8} RUG PULL WARNING: This token has been flagged as potentially rugged!
`;
      }
      if (source !== "telegram" && topPercentHolders.length > 3) {
        analysisText += `\u{1F465} Top Holders: ${formatPercentages(
          topPercentHolders
        )}
`;
      }
...
`;
      ...
      if (tokenData.topWallets && tokenData.topWallets.length > 0) {
        ...
        if (source === "telegram") {
          analysisText += `
\u{1F4CA} Top Wallets Trading This Token:
`;
          tokenData.topWallets.slice(0, walletsToShow).forEach((wallet, index) => {
...
`;
            if (wallet.historic30d) {
              ...
              const changeIcon = change >= 0 ? "\u{1F4C8}" : "\u{1F4C9}";
...
`;
          ...
          analysisText += `\u{1F4B9} Top Wallet: ${name} (${topWallet.winrate}% WR)`;
      ...
      analysisText += `
\u{1F50D} View more top wallets: https://www.topwallets.ai/solana/token/${address}
`;
      analysisText += `
\u{1F50D} View detailed chart: ${chartUrl}`;
      await callback({
        text: analysisText
      });
      return true;
    } catch (error) {
      elizaLogger6.error("Token scan error", {
        error,
        address,
        errorMessage: isAxiosError2(error) ? error.response?.data?.message || error.message : error instanceof Error ? error.message : "Unknown error",
        isAxiosError: isAxiosError2(error)
      });
      const errorMessage = isAxiosError2(error) ? `Failed to scan token: ${error.response?.data?.message || error.message}` : "An unexpected error occurred while scanning the token.";
      console.log(error);
      if (source === "telegram") {
        await callback({
          text: errorMessage
...
__export(actions_exports, {
  scanWalletAction: () => scanWalletAction
});

// src/providers/index.ts
var providers_exports = {};
__export(providers_exports, {
  trendingTokensProvider: () => trendingTokensProvider
});

// src/index.ts
var topwalletsPlugin = {
  name: "topwallets",
  description: "A plugin for Eliza that provides solana trading data and analysis.",
